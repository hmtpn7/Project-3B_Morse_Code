...
public:
	...
	*/
	Binary_Tree(const Item_Type& the_data,	const Item_Type& code,
		const Binary_Tree<Item_Type>& left_child	= Binary_Tree(),
		const Binary_Tree<Item_Type>& right_child	= Binary_Tree()) :
		root(new BTNode<Item_Type>(the_data, code, left_child.root,
		right_child.root)) {}
	...
	*/
	...
	Binary_Tree<Item_Type> read_binary_tree(std::vector<std::string>& text, int& i, int& a);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//  Creator : Hung Tran																				    
//        Read_Tree Function wrapper (public field)
//			import node to tree from vector
//////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Item_Type>
void Binary_Tree<Item_Type>::read_tree(std::vector<std::string>& text) {
	vector<std::string>::iterator it = text.begin();
	int n = 0;
	int a = 0;
	string cd = "";
	Binary_Tree<Item_Type> newTree = read_binary_tree(text, n, a);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//  Creator : Hung Tran																				    
//        Read_Tree recursive Function (private field)
//			import node to tree from vector
//////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Item_Type>
Binary_Tree<Item_Type> Binary_Tree<Item_Type>::
read_binary_tree(std::vector<std::string>& text, int& i, int& a) {
	...
	if (i>text.size() - 1 || text[i] == "NULL") {
		...
		std::string txtcode = "";
		if (text[i] != txt) {
			txt.push_back(text[i][0]);
			std::string tx(text[i].begin() + 1, text[i].end());
			txtcode = tx;

		}
		

		Binary_Tree<Item_Type>left = read_binary_tree(text, ++i, a);
		
		Binary_Tree<Item_Type>right = read_binary_tree(text, ++i, a);
		
	
		return Binary_Tree<Item_Type>(txt, txtcode, left, right);
		
		
	}
}
//  root (null, null, left, right)
//       /             \
//      /               \
//     /                 \
//  left                 right
//(txt, code, left)     (txt, code, right)
//tạo 1 function mới tách riêng left và right sau đó nhập lại ở function chính.
